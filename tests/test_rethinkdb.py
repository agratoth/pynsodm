...
def test_init_table(mock_server):
    ...
    storage.reconnect()
    table_list = storage._driver\
        .db(test_db_name)\
        .table_list()\
        .run(storage._connection)
...
def test_init_index(mock_server):
    ...
    storage.reconnect()
    index_list = storage._driver\
        .db(test_db_name)\
        .table(User.table_name)\
        .index_list().run(storage._connection)
...
def test_complex_relations(mock_server):
    ...
    bike2 = Bike(
        model='Bicystar Explorer 26"',
        number='bike456',
        owner=person1)
    bike2.save()

    bike3 = Bike(
        model='Horn Forest FHD 7.1 27.5',
        number='bike789',
        owner=person2)
    bike3.save()
    ...
    assert \
        finded_bike1.owner.idcard.number == 'test123' and \
        finded_bike2.owner.idcard.number == 'test456'
